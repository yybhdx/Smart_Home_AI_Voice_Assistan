...
烧录完成后，**ESP32-S3 的 BOOT 键（GPIO 0）** 被定义为多功能操作键，用于在不同工作模式间切换：

1.  **激活 AI 模式（手动开启对话）**：
    *   在设备空闲时按下 BOOT 键，程序会主动发起与小智服务器的连接。
    *   **日志表现**：会出现 `STATE: connecting` 和 `Session ID: xxx`。
    *   **关键变化**：你会看到 `wifi:Set ps type: 0, coexist: 0`。这表示**关闭 WiFi 省电模式**，进入高性能状态，以确保语音流传输的低延迟。

2.  **退出 AI 模式（进入后台模式）**：
    *   再次按下 BOOT 键，设备会断开语音会话。
    *   **日志表现**：会出现 `MQTT: Received goodbye message`。
    *   **关键变化**：你会看到 `wifi:Set ps type: 1, coexist: 0`。这表示**开启 WiFi 省电模式**，降低功耗。

3.  **后台持续任务（华为云上传）**：
    *   无论小智 AI 是否在工作，**传感器数据上传华为云的任务都是独立且持续运行的**。
    *   即使你按键关闭了 AI 对话（ps type: 1），ESP32-S3 依然会每秒从 STM32 读取数据并推送到华为云平台，确保移动端 APP 监控不中断。

#### 总结：BOOT 键的作用
*   **作为开关**：手动开启或强制结束语音对话。
*   **性能切换**：在“高性能语音模式 (ps type: 0)”与“低功耗后台监控模式 (ps type: 1)”之间切换。
